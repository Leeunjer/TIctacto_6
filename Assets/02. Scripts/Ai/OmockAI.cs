...
public class OmockAI : MonoBehaviour
{
    ...
    public static async Task<(int row, int col)?> GetBestMove(Constans.PlayerType[,] board)
    {
        return await Task.Run(() =>
        {
            ...
            for (var row = 0; row < board.GetLength(0); row++)
            {
                ...
                {
                    if (board[row, col] == Constans.PlayerType.None && IsMinimaxValidPosition(row, col, board))
                    {
                        board[row, col] = Constans.PlayerType.Player2;
                        var score = DoMinimax(board, 0, false, alpha, beta);

                        PrintBoard(board);

                        board[row, col] = Constans.PlayerType.None;
    ...
    private static float DoMinimax(Constans.PlayerType[,] board, int depth, bool isMaximizing, float alpha, float beta)
    {
        if (CheckGameWin(Constans.PlayerType.Player1, board))
            return -MAX_SCORE;
        if (CheckGameWin(Constans.PlayerType.Player2, board))
            return MAX_SCORE;
        ...
        else
        {
            ...
            for (var row = 0; row < board.GetLength(0); row++)
            {
                for (var col = 0; col < board.GetLength(1); col++)
                {
                    if (board[row, col] == Constans.PlayerType.None && IsMinimaxValidPosition(row, col, board))
                    {
                        board[row, col] = Constans.PlayerType.Player1;
    ...
    private static float EvaluateBoard(Constans.PlayerType[,] board)
    {
        float score = 0;
        ...
        for (int row = 0; row < rows; row++)
        {
            ...
            {
                if (board[row, col] == Constans.PlayerType.None)
                    continue;

                bool isAI = board[row, col] == Constans.PlayerType.Player2;
    ...
    private static float EvaluatePattern(Constans.PlayerType[,] board, int startRow, int startCol, int dRow, int dCol, int length)
    {
        ...
        for (int i = 1; i < length; i++)
        {
            ...
            else if (board[newRow, newCol] == Constans.PlayerType.None)
            {
                emptyAfter++;
                break;
            }
        ...
        for (int i = 1; i < length; i++)
        {
            ...
            if (board[newRow, newCol] == Constans.PlayerType.None)
            {
                emptyBefore++;
                break;
            }
    ...
    private static bool IsValidPosition(int row, int col, Constans.PlayerType[,] board)
    {
    ...
    private static bool IsMinimaxValidPosition(int row, int col, Constans.PlayerType[,] board)
    {
        if ((row > 0 && col > 0 && board[row - 1, col - 1] != Constans.PlayerType.None) ||
            (row > 0 && board[row - 1, col] != Constans.PlayerType.None) ||
            (row > 0 && col < board.GetLength(1) - 1 && board[row - 1, col + 1] != Constans.PlayerType.None) ||
            (col > 0 && board[row, col - 1] != Constans.PlayerType.None) ||
            (col < board.GetLength(1) - 1 && board[row, col + 1] != Constans.PlayerType.None) ||
            (row < board.GetLength(0) - 1 && col > 0 && board[row + 1, col - 1] != Constans.PlayerType.None) ||
            (row < board.GetLength(0) - 1 && board[row + 1, col] != Constans.PlayerType.None) ||
            (row < board.GetLength(0) - 1 && col < board.GetLength(1) - 1 && board[row + 1, col + 1] != Constans.PlayerType.None))
        {
            return true;
        }
        return false;
    ...
    public static bool IsAllBlocksPlaced(Constans.PlayerType[,] board)
    {
        for (var row = 0; row < board.GetLength(0); row++)
        {
            for (var col = 0; col < board.GetLength(1); col++)
    ...
    public static bool CheckGameWin(Constans.PlayerType playerType, Constans.PlayerType[,] board)
    {
        // None 타입은 승리 조건에서 제외
        if (playerType == Constans.PlayerType.None)
            return false;
    ...
    public static void PrintBoard(Constans.PlayerType[,] board)
    {
        int rows = board.GetLength(0);
        int cols = board.GetLength(1);
